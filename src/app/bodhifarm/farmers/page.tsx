    ...
    try {
      const [
        ...
        supabase
          .from('farms')
          .select(
            'id, farmer_id, farm_name, farm_type, status'
          ),
        ...
        supabase
          .from('feed_records')
          .select(
            'id, farmer_id, quantity_kg, total_cost'
          ),
      ...
      setFarmers(
        (farmersResult.data || []) as Farmer[]
      );

      setFarms(
        (farmsResult.data || []) as Farm[]
      );

      setBatches(
        (batchesResult.data || []) as BirdBatch[]
      );

      setEggs(
        (eggsResult.data || []) as EggRecord[]
      );

      setFeeds(
        (feedsResult.data || []) as FeedRecord[]
      );
  ...
  const summaries = useMemo<FarmerSummary[]>(() => {
    return farmers.map((farmer) => {
      ...
      const farmerEggs = eggs.filter(
        (egg) =>
          egg.farmer_id === farmer.id
      );

      const farmerFeeds = feeds.filter(
        (feed) =>
          feed.farmer_id === farmer.id
      );

      const farmerVeterinary =
        veterinary.filter(
          (record) =>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <Metric
                            label="Veterinary"
                            value={
                              selectedFarmer.veterinaryRecords
                            }
                          />
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <Metric
                            label="Saleable Eggs"
                            value={
                              selectedFarmer.saleableEggs
                            }
                          />

                          <Metric
                            label="Feed"
                            value={`${selectedFarmer.feedKg.toFixed(
                              2
                            )} kg`}
                          />
                          ...
                          <Metric
                            label="Feed Cost"
                            value={`₹${selectedFarmer.feedCost.toFixed(
                              2
                            )}`}
                          />
